...
5.      :type if 1==0 then 'a' else "a"
        <interactive>:1:23:
        Couldn't match expected type `Char' with actual type `[Char]'
        In the expression: "a"
        In the expression: if 1 == 0 then 'a' else "a"

  GHCi espera los tipos de `expr1` y `expr2` en una expresión condicional

        if test then expr1 else expr2

  Es lo mismo. Pero un carácter no es una lista de caracteres, por lo que la expresión condicional, aunque cumple con las reglas de la sintaxis de Haskell, no está bien formada.